...
def _appeals_keyboard(user_id):
    ...
    kb.add(PremiumButton(get_text(user_id, 'btn_back_menu', users), '🔙', '5904258298764334001', callback_data='main_menu'))
    return kb


def _appeals_menu_text(user_id):
    from bot_core import BOT_NAME
    return get_text(user_id, 'appeals_menu_text', users).format(bot_name=BOT_NAME)
...
def handle_appeal_suggest(call):
    ...
    text = get_text(user_id, 'appeal_suggest_text', users)
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(PremiumButton(get_text(user_id, 'btn_back_menu', users), '🔙', '5904258298764334001', callback_data='main_menu'))
...
def handle_appeal_complain(call):
    ...
    text = get_text(user_id, 'appeal_complain_text', users)
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(PremiumButton(get_text(user_id, 'btn_back_menu', users), '🔙', '5904258298764334001', callback_data='main_menu'))
    send_photo_message(chat_id, message_id, text, kb)
    bot.answer_callback_query(call.id)
