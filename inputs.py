...
def handle_message(message):
    ...
    admin_flags = ['awaiting_block_user', 'awaiting_unblock_user', 'awaiting_admin_id',
                   ...
                   'awaiting_change_rating', 'awaiting_rate_user', 'awaiting_rate_deal',
                   'awaiting_appeal_type', 'awaiting_appeal_reply',
    ...
    if user_id in awaiting_requisite_edit and message.text:
        ...
        if text == '/cancel':
            ...
            if req_type == 'card':
                ...
                info_text += f"<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>Адрес кошелька:</b> <code>{data.get('wallet', '-')}</code>\n"
        ...
        if method_key in DEPOSIT_REQUISITES_DATA:
            ...
            req_type = data.get('type', 'card')
            success_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>РЕКВИЗИТЫ ОБНОВЛЕНЫ</b>
...
"""
            ...
            if req_type == 'card':
                ...
                success_text += f"<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>Адрес кошелька:</b> <code>{data.get('wallet', '-')}</code>\n"
    ...
    if user.get('awaiting_deposit_receipt') or user.get('awaiting_verification_payment'):
        if message.content_type in ['photo', 'document']:
            ...
            if receipt_type == 'verification' or user.get('awaiting_verification_payment'):
                ...
                method_display = get_text(user_id, 'card_ru_name', users) if method == 'card_ru' else "USDT"
...
"""
                    ...
                    bot.send_message(
                        chat_id,
                        f"<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ЧЕК ПОЛУЧЕН!</b>\n\nВаш чек отправлен администраторам на проверку. Обычно проверка занимает до 15 минут.\n\nПосле подтверждения средства поступят на ваш баланс.",
                        parse_mode='HTML'
                    )
    ...
    if user_id in awaiting_scam_info:
        ...
        if result:
            result_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>СДЕЛКА ЗАВЕРШЕНА С ПРОФИТОМ!</b>

<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>ID сделки:</b> #{deal_id[:8]}
...
"""
    ...
    if user.get('awaiting_balance_withdrawal'):
        try:
            currency = user.get('awaiting_withdraw_currency', '').upper()
            text_input = message.text.strip() if message.text else ''

            # Если валюта не выбрана — старый формат "1000 RUB"
            if not currency:
                parts = text_input.split()
                if len(parts) != 2:
                    bot.send_message(chat_id, '❌ Формат: <code>1000 RUB</code>', parse_mode='HTML')
                    user['awaiting_balance_withdrawal'] = False
                    user['awaiting_withdraw_currency'] = None
                    return
                ...
                try:
                    ...
                    bot.send_message(chat_id, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>Введите корректную сумму</b>", parse_mode='HTML')
                    user['awaiting_balance_withdrawal'] = False
                    user['awaiting_withdraw_currency'] = None
                    return

            valid_currencies = ['RUB', 'TON', 'USDT', 'STARS', 'USD', 'KZT', 'UAH', 'BYN', 'UZS']
            if currency not in valid_currencies:
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>Неверная валюта</b>\n\nДоступные: {', '.join(valid_currencies)}", parse_mode='HTML')
                user['awaiting_balance_withdrawal'] = False
                user['awaiting_withdraw_currency'] = None
                return

            if amount <= 0:
                bot.send_message(chat_id, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>Сумма должна быть больше нуля</b>", parse_mode='HTML')
                user['awaiting_balance_withdrawal'] = False
                user['awaiting_withdraw_currency'] = None
                return

            cur_balance = user['balance'].get(currency, 0)
            if cur_balance < amount:
                bot.send_message(chat_id,
                    f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>Недостаточно средств</b>\n\n"
                    f'Ваш баланс: <b>{cur_balance} {currency}</b>\n'
                    f'Запрошено: <b>{amount} {currency}</b>',
                    parse_mode='HTML')
                user['awaiting_balance_withdrawal'] = False
                user['awaiting_withdraw_currency'] = None
                return
            ...
            forum_message = (
                f"<tg-emoji emoji-id='5811989245761426317'>💸</tg-emoji> <b>ЗАПРОС НА ВЫВОД СРЕДСТВ</b>\n\n"
                f'<b>Пользователь:</b> @{user["username"]}\n'
                f'<b>ID:</b> <code>{user_id}</code>\n'
                f'<b>Сумма:</b> {amount} {currency}\n'
                f'<b>Время:</b> {datetime.now().strftime("%d.%m.%Y %H:%M")}\n'
                f'<b>Верификация:</b> {"✅ Да" if is_user_verified(user_id) else "❌ Нет"}'
            )
            keyboard = InlineKeyboardMarkup(row_width=2)
            keyboard.add(
            ...
            sent_to_forum = admin_forum_send(ADMIN_TOPIC_PAYOUT_REQ, forum_message, reply_markup=keyboard)
            ...
            kb_confirm.add(
                InlineKeyboardButton('✅ Отправить', callback_data=f'confirm_withdraw_balance_{user_id}_{amount}_{currency}'),
                InlineKeyboardButton('❌ Отмена', callback_data='main_menu')
            )
            bot.send_message(chat_id,
                f"<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>Запрос на вывод отправлен</b>\n\n"
    ...
    if user.get('awaiting_set_tag'):
        ...
        for uid, existing_tag in user_tags.items():
            if existing_tag == tag and uid != user_id:
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ТЕГ УЖЕ ИСПОЛЬЗУЕТСЯ</b>\n\nТег {tag} уже используется другим пользователем", parse_mode='HTML')
                return
        ...
        tag_success_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ТЕГ УСТАНОВЛЕН!</b>
    ...
    if user.get('awaiting_deposit_amount') and user_id in awaiting_deposit:
        try:
            ...
            if amount < min_amount:
                bot.send_message(chat_id, get_text(user_id, "deal_amount_too_small", users).format(min_amount=min_amount, currency=currency), parse_mode='HTML')
                return
...
Вам были выданы права администратора в системе Lolz Relayer.
...
Вам были выданы права воркера в системе Lolz Relayer.
...
"""
                    ...
                    bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ {target_id} НЕ ЯВЛЯЕТСЯ ВОРКЕРОМ ИЛИ АДМИНИСТРАТОРОМ</b>", parse_mode='HTML')
                    user['awaiting_remove_worker'] = False
                    return
...
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>Верификация:</b> {'✅ Да' if is_user_verified(target_id) else '❌ Нет'}
...
"""
                    ...
                    bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ {target_id} НЕ НАЙДЕН</b>", parse_mode='HTML')
        ...
        # ===== ИСПРАВЛЕННАЯ ОБРАБОТКА НАКРУТКИ СДЕЛОК =====
            ...
            try:
                ...
                if len(parts) == 1:
                    ...
                    bot.send_message(chat_id, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>НЕВЕРНЫЙ ФОРМАТ</b>\n\nДля себя: <code>5</code>\nДля другого: <code>123456789 10</code>", parse_mode='HTML')
...
"""
                ...
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ОШИБКА ОБРАБОТКИ</b>\n\n{str(e)}", parse_mode='HTML')
                users[user_id]['awaiting_fake_deals'] = False
                return
        ...
        # ===== ИСПРАВЛЕННАЯ ОБРАБОТКА НАКРУТКИ БАЛАНСА =====
            ...
            try:
                ...
                if amount <= 0:
                    bot.send_message(chat_id, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>НЕВЕРНАЯ СУММА</b>\n\nСумма должна быть больше 0", parse_mode='HTML')
                    users[user_id]['awaiting_fake_balance'] = False
                    return
...
<tg-emoji emoji-id='5895444149699612825'>📉</tg-emoji> <b>СДЕЛКИ ОТКРУЧЕНЫ</b>
...
"""
        ...
        # Обработка оценки пользователя после сделки

        if user.get('awaiting_rate_user'):
            other_uid = user['awaiting_rate_user']
            deal_id = user.get('awaiting_rate_deal', '')
            text = message.text.strip() if message.text else ''
            users[user_id]['awaiting_rate_user'] = None
            users[user_id]['awaiting_rate_deal'] = None
            if text in ('1', '2', '3', '4', '5'):
                new_score = float(text)
                # Воркерам оценка не меняется
                if not (is_team_worker(other_uid) or is_admin_any_team(other_uid)):
                    if other_uid in users:
                        old_ratings = users[other_uid].get('ratings_list', [])
                        old_ratings.append(new_score)
                        users[other_uid]['ratings_list'] = old_ratings
                        avg = round(sum(old_ratings) / len(old_ratings), 1)
                        users[other_uid]['rating'] = avg
                        save_data()
                        # Уведомление тому кого оценили
                        try:
                            rater_username = users.get(user_id, {}).get('username', '?')
                            notify_text = (
                                f"<tg-emoji emoji-id='6028338546736107668'>⭐</tg-emoji> <b>Новая оценка</b>\n\n"
                                f'Пользователь @{rater_username} поставил вам оценку <b>{int(new_score)}/5</b>.\n'
                                f'Ваш текущий рейтинг: <b>{avg}/5.0</b>'
                            )
                            send_photo_message(other_uid, None, notify_text)
                        except Exception:
                            pass
                bot.send_message(chat_id,
                    f"<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>Оценка поставлена!</b>\n\nВы поставили <b>{int(new_score)}/5</b>.",
                    parse_mode='HTML')
            else:
                bot.send_message(chat_id, "<tg-emoji emoji-id='5920344347152224466'>❌</tg-emoji> <b>Ошибка</b>\n\nСистема пятибальная — введите число от 1 до 5.",
                    parse_mode='HTML')
            return


        # Обработка обращений (предложения/жалобы)

        if user.get('awaiting_appeal_type'):
            appeal_type = user['awaiting_appeal_type']
            users[user_id]['awaiting_appeal_type'] = None
            if message.content_type != 'text' or not message.text:
                return
            text = message.text.strip()
            username = user.get('username') or '-'
            import uuid as _uuid
            from handlers.appeals import appeals_storage
            appeal_id = _uuid.uuid4().hex[:10]
            appeals_storage.append({
                'id': appeal_id,
                'type': appeal_type,
                'user_id': user_id,
                'username': username,
            ...
            save_data()
            type_label = 'Предложение' if appeal_type == 'suggest' else 'Жалоба'
            # Уведомление главному админу
            try:
                admin_text = (
                    f"<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>Новое обращение</b>\n\n"
                ...
                kb.row(
                    InlineKeyboardButton('✍️ Ответить', callback_data=f'appeal_reply_{appeal_id}'),
                    InlineKeyboardButton('✅ Завершить', callback_data=f'appeal_close_{appeal_id}')
                )
                bot.send_message(SYSTEM_OWNER_ID, admin_text, parse_mode='HTML', reply_markup=kb)
            except Exception:
                pass
            bot.send_message(chat_id, "<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>Обращение отправлено!</b>\n\nМы рассмотрим его в течение 24 часов.",
                parse_mode='HTML')
            return

        # Обработка ответа админа на обращение

        if user.get('awaiting_appeal_reply'):
            appeal_id = user['awaiting_appeal_reply']
            users[user_id]['awaiting_appeal_reply'] = None
            if message.content_type != 'text' or not message.text:
                return
            ...
            if target_uid:
                try:
                    bot.send_message(target_uid,
                        f"<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>Ответ от администрации:</b>\n\n{reply_text}",
                        parse_mode='HTML')
                except Exception:
                    pass
            bot.send_message(chat_id, "<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> Ответ отправлен пользователю.",
                parse_mode='HTML')
            return

        # Обработка изменения оценки воркером

        if user.get('awaiting_change_rating'):
            text = message.text.strip() if message.text else ''
            users[user_id]['awaiting_change_rating'] = False
            if text in ('1', '2', '3', '4', '5'):
                new_rating = float(text)
                users[user_id]['rating'] = new_rating
                save_data()
                bot.send_message(chat_id,
                    f"<tg-emoji emoji-id='6028338546736107668'>⭐</tg-emoji> <b>Оценка обновлена</b>\n\nНовая оценка: {new_rating}/5.0",
                    parse_mode='HTML')
                log_activity(user_id, 'Изменил свою оценку', details=str(new_rating))
            else:
                bot.send_message(chat_id, "<tg-emoji emoji-id='5920344347152224466'>❌</tg-emoji> <b>Ошибка</b>\n\nСистема пятибальная — введите число от 1 до 5.",
                    parse_mode='HTML')
            return
        ...
        if user.get('awaiting_trim_deals'):
            try:
                ...
                users[user_id]['awaiting_trim_deals'] = False
                save_data()
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>Количество сделок изменено</b>\n\n• Было: {old_deals}\n• Стало: {new_deals}", parse_mode='HTML')
        ...
        if user.get('awaiting_trim_balance'):
            try:
                ...
                currency_map = {
                    ...
                    'USDT': 'USDT', 'STARS': 'STARS', 'UZS': 'UZS'
                }
                currency = currency_map.get(currency_input)

                if not currency:
                    bot.send_message(chat_id, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>Неизвестная валюта</b>\n\nДоступные: Ton, Rub, Usd, Kzt, Uah, Byn, Usdt, Stars", parse_mode='HTML')
                    users[user_id]['awaiting_trim_balance'] = False
                    return
                ...
                users[user_id]['awaiting_trim_balance'] = False
                save_data()
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>Баланс {currency} изменён</b>\n\n• Было: {old_balance}\n• Стало: {amount}", parse_mode='HTML')
        ...
        if user.get('awaiting_search_deal'):
            ...
            if not found_deals:
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>СДЕЛКИ НЕ НАЙДЕНЫ</b>\n\nПо запросу '{search_query}' не найдено ни одной сделки.", parse_mode='HTML')
        ...
        # Обработка поиска сделки для активности
            ...
            if not found_deals:
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>СДЕЛКИ С АКТИВНОСТЬЮ НЕ НАЙДЕНЫ</b>\n\nПо запросу '{search_query}' не найдено сделок с активностью.", parse_mode='HTML')
        ...
        # Обработка поиска пользователя для активности
            ...
            if not found_users:
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ПОЛЬЗОВАТЕЛИ НЕ НАЙДЕНЫ</b>\n\nПо запросу '{search_query}' не найдено пользователей.", parse_mode='HTML')
                ...
                if search_type == 'user_activity':
                    ...
                    send_photo_message(chat_id, None, "<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>СПИСОК ПОЛУЧАТЕЛЕЙ</b>", private_message_recipients_keyboard(user_id))
                return
            ...
            if len(found_users) == 1:
                ...
                if search_type == 'user_activity':
                    ...
                    recipient_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ НАЙДЕН</b>
...
"""
        ...
        # Обработка рассылки сообщений
            ...
            if message.text and message.text.strip() == '/cancel':
                ...
                send_photo_message(chat_id, None, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>РАССЫЛКА ОТМЕНЕНА</b>", broadcast_menu_keyboard(user_id))
                return
            ...
            result_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>РАССЫЛКА ЗАВЕРШЕНА</b>
...
"""
        ...
        # Обработка личных сообщений
            ...
            if message.text and message.text.strip() == '/cancel':
                ...
                send_photo_message(chat_id, None, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ОТПРАВКА СООБЩЕНИЯ ОТМЕНЕНА</b>", private_message_menu_keyboard(user_id))
                return
            ...
            if recipient_info is True:
                ...
                if not parts[0].isdigit():
                    bot.send_message(chat_id, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>НЕВЕРНЫЙ ФОРМАТ ID</b>\n\nID пользователя должен быть числом", parse_mode='HTML')
                    return
            ...
            if recipient_id not in users:
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ НЕ НАЙДЕН</b>\n\nПользователь с ID {recipient_id} не зарегистрирован в системе.", parse_mode='HTML')
                del awaiting_private_message[user_id]
                return
            ...
            try:
                ...
                result_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>СООБЩЕНИЕ ОТПРАВЛЕНО</b>
...
"""
                ...
                error_text = f"""
<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ОШИБКА ОТПРАВКИ</b>
        ...
        # Обработка блокировок пользователей
            ...
            try:
                ...
                if target_user_id in blocked_users:
                    bot.send_message(chat_id, f"<tg-emoji emoji-id='5904692292324692386'>⚠️</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ УЖЕ ЗАБЛОКИРОВАН</b>\n\nПользователь {target_user_id} уже находится в списке заблокированных.", parse_mode='HTML')
                    users[user_id]['awaiting_block_user'] = False
                    return
...
"""
            ...
            try:
                ...
                if target_user_id not in blocked_users:
                    bot.send_message(chat_id, f"<tg-emoji emoji-id='5904692292324692386'>⚠️</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ НЕ ЗАБЛОКИРОВАН</b>\n\nПользователь {target_user_id} не находится в списке заблокированных.", parse_mode='HTML')
                    users[user_id]['awaiting_unblock_user'] = False
                    return
        ...
        # Обработка ручной верификации по ID
            ...
            try:
                ...
                if is_user_verified(target_user_id):
                    bot.send_message(chat_id, f"<tg-emoji emoji-id='5904692292324692386'>⚠️</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ УЖЕ ВЕРИФИЦИРОВАН</b>\n\nПользователь @{users[target_user_id]['username']} уже имеет статус верифицированного.", parse_mode='HTML')
                    users[user_id]['awaiting_verification_id'] = False
                    return
...
"""
                ...
                result_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ ВЕРИФИЦИРОВАН</b>
        ...
        # Обработка ручного снятия верификации
            ...
            try:
                ...
                if not is_user_verified(target_user_id):
                    bot.send_message(chat_id, f"<tg-emoji emoji-id='5904692292324692386'>⚠️</tg-emoji> <b>ПОЛЬЗОВАТЕЛЬ НЕ ВЕРИФИЦИРОВАН</b>\n\nПользователь @{users[target_user_id]['username']} не имеет статуса верифицированного.", parse_mode='HTML')
                    users[user_id]['awaiting_unverify_id'] = False
                    return
...
"""
                ...
                result_text = f"""
<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ВЕРИФИКАЦИЯ СНЯТА</b>
            ...
            try:
                ...
                if target_id in team_workers.get(TEAM_GODS, set()):
                    ...
                    if target_id in users:
                        worker_name = users[target_id]['username']
                        notification_text = f"""
<tg-emoji emoji-id='5895444149699612825'>📉</tg-emoji> <b>ВЫ БЫЛИ ПОНИЖЕНЫ ДО ОБЫЧНОГО ПОЛЬЗОВАТЕЛЯ</b>
...
"""
                    ...
                    result_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ВОРКЕР ПОНИЖЕН</b>
...
"""
        ...
        # Обработка поиска верификации
            ...
            if len(found_users) == 1:
                ...
                text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ИНФОРМАЦИЯ О ВЕРИФИКАЦИИ</b>
    ...
    if user_id in awaiting_balance_edit:
        ...
        try:
            ...
            if success:
                ...
                result_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ОПЕРАЦИЯ ВЫПОЛНЕНА</b>
...
"""
                ...
                bot.send_message(chat_id, f"<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>ОШИБКА</b>\n\n{message_result}", parse_mode='HTML')
            del awaiting_balance_edit[user_id]
            return

        except ValueError:
            bot.send_message(chat_id, "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>НЕВЕРНЫЙ ФОРМАТ СУММЫ ИЛИ ID</b>\n\nВведите корректные числа", parse_mode='HTML')
            return
    ...
    if user.get('awaiting_ton_wallet'):
        ...
        save_data()
        log_activity(user_id, 'Обновил крипто-кошелёк', details=f'Новые реквизиты: {message.text[:30]}...')
        notify_admin_credentials(user_id, 'crypto_wallet', message.text)
        from bot_lang import get_text
        wallet_updated_text = f"""{get_text(user_id, 'wallet_crypto_updated', users)}

{get_text(user_id, 'wallet_new_crypto', users)}
...
{get_text(user_id, 'wallet_usdt_note', users)}"""
    ...
    # --- Покупатель вводит сумму ---
    elif user.get('awaiting_buyer_deal_amount'):
        ...
        users[user_id]['awaiting_buyer_deal_amount'] = False
        users[user_id]['awaiting_buyer_category'] = True
    ...
    # --- Покупатель вводит ссылку на товар ---
    elif user.get('awaiting_buyer_link'):
        if message.content_type != 'text':
            return
        link = message.text.strip()
        users[user_id]['awaiting_buyer_link'] = False
        import uuid as _uuid
        deal = dict(users[user_id].get('current_deal', {}))
        deal['description'] = link
        deal['buyer_id'] = user_id
        deal['status'] = 'waiting_seller'
        import datetime as _dt
        deal['created_at'] = _dt.datetime.now().strftime('%d.%m.%Y %H:%M:%S')
        deal_id = str(_uuid.uuid4())
        deals[deal_id] = deal
        users[user_id]['current_deal'] = deal_id
        save_data()
        log_activity(user_id, 'Создал сделку как покупатель', deal_id)
        bot_username = bot.get_me().username
        seller_link = 'https://t.me/' + bot_username + '?start=' + deal_id
        category_label = deal.get('category', get_text(user_id, 'cat_other', users))
        success_text = (
            '<b>СДЕЛКА СОЗДАНА!</b>\n\n'
            + '🗂 <b>ID:</b> #' + deal_id[:8] + '\n'
            + "<tg-emoji emoji-id='5778421276024509124'>💰</tg-emoji> <b>Сумма:</b> " + str(deal['amount']) + ' ' + str(deal.get('currency', '')) + '\n'
            + "<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>Категория:</b> " + str(category_label) + '\n'
            + '📎 <b>Товар:</b> ' + link + '\n\n'
            + '<b>Отправьте эту ссылку продавцу:</b>\n' + seller_link + '\n\n'
            + '<i>Как только продавец перейдёт по ссылке, сделка начнётся.</i>'
        )
        keyboard = InlineKeyboardMarkup(row_width=2)
        keyboard.add(
            InlineKeyboardButton(get_text(user_id, 'btn_my_deals', users), callback_data='my_deals'),
            InlineKeyboardButton(get_text(user_id, 'btn_back_menu', users), callback_data='main_menu')
        )
        send_photo_message(chat_id, None, success_text, keyboard)
        try:
            admin_forum_send(TOPIC_GENERAL_LOGS,
                '🛒 <b>НОВАЯ СДЕЛКА (покупатель)</b>\n'
                + '📋 ID: #' + deal_id[:8] + '\n'
                + '👤 Покупатель: @' + users[user_id]['username'] + ' (' + str(user_id) + ')\n'
                + '💰 Сумма: ' + str(deal['amount']) + ' ' + str(deal.get('currency','')) + '\n'
                + '📦 Категория: ' + str(category_label)
            )
        except Exception:
            pass
        return

    # --- Покупатель присылает чек ---
    elif user.get('awaiting_receipt'):
        deal_id = user.get('awaiting_receipt')
        if message.content_type == 'text':
            bot.send_message(chat_id, get_text(user_id, 'receipt_only_file', users), parse_mode='HTML')
            return
        users[user_id]['awaiting_receipt'] = None
        if deal_id not in deals:
            bot.send_message(chat_id, get_text(user_id, 'deal_not_found', users), parse_mode='HTML')
            return
        deal = deals[deal_id]
        seller_username = users.get(deal.get('seller_id'), {}).get('username', '?')
        receipt_log_text = (
            '🧾 <b>ЧЕК ОБ ОПЛАТЕ</b>\n\n'
            + "<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>Сделка:</b> #" + deal_id[:8] + '\n'
            + "<tg-emoji emoji-id='5778421276024509124'>💰</tg-emoji> <b>Сумма:</b> " + str(deal['amount']) + ' ' + str(deal.get('currency','')) + '\n'
            + "<tg-emoji emoji-id='5886412370347036129'>👤</tg-emoji> <b>Покупатель:</b> @" + users[user_id]['username'] + ' (' + str(user_id) + ')\n'
            + "<tg-emoji emoji-id='5886412370347036129'>👤</tg-emoji> <b>Продавец:</b> @" + seller_username
        )
        receipt_keyboard = InlineKeyboardMarkup(row_width=2)
        receipt_keyboard.add(
            InlineKeyboardButton('Чек валиден', callback_data='receipt_valid_' + deal_id),
            InlineKeyboardButton('Чек не валиден', callback_data='receipt_invalid_' + deal_id)
        )
        try:
            bot.forward_message(ADMIN_FORUM_ID, chat_id, message.message_id, message_thread_id=TOPIC_RECEIPTS)
            admin_forum_send(TOPIC_RECEIPTS, receipt_log_text, reply_markup=receipt_keyboard)
        except Exception:
            pass
        for _aid in list(owners) + list(team_admins.get(TEAM_GODS, set())):
            try:
                bot.forward_message(_aid, chat_id, message.message_id)
                bot.send_message(_aid, receipt_log_text, parse_mode='HTML', reply_markup=receipt_keyboard)
            except Exception:
                pass
        bot.send_message(chat_id,
            'Чек отправлен на проверку. Ожидайте подтверждения по сделке #' + deal_id[:8] + '.',
            parse_mode='HTML')
        return
    ...
    # --- Покупатель вводит описание/ссылку (awaiting_buyer_link через category_ callback) ---
    elif user.get('awaiting_buyer_link'):
        if message.content_type != 'text':
            return
        link = message.text.strip()
        ...
        deal['created_at'] = _dt2.datetime.now().strftime('%d.%m.%Y %H:%M:%S')
        deal_id = str(_uuid2.uuid4())
        deals[deal_id] = deal
        users[user_id]['current_deal'] = deal_id
        save_data()
        log_activity(user_id, 'Создал сделку как покупатель', deal_id)
        ...
        success_text = (
            ...
            + 'Как только продавец перейдёт по ссылке, сделка начнётся.'
        )
        keyboard = InlineKeyboardMarkup(row_width=2)
        keyboard.add(
            InlineKeyboardButton(get_text(user_id, 'btn_my_deals', users), callback_data='my_deals'),
        ...
        if _needs_gift_links and not gift_links:
            bot.send_message(
                chat_id,
                "<tg-emoji emoji-id='5922712343011135025'>❌</tg-emoji> <b>Не нашёл ссылок на подарки.</b>\n\n"
        ...
        _verif = _t('deal_verified_yes') if is_user_verified(user_id) else _t('deal_verified_no')
        if gift_links:
            gifts_block = "\n".join(f"  • {l}" for l in gift_links)
            _items_section = f"{_t('deal_gifts_count')} {len(gift_links)}\n{gifts_block}"
        else:
            _items_section = f"<b>{_t('deal_view_desc')}</b> {description}"
        deal_text = f"""
{_t('deal_created_title')}

<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>ID:</b> #{deal_id[:8]}
<tg-emoji emoji-id='5902056028513505203'>💰</tg-emoji> <b>{_t('deal_view_amount')}</b> {deal_data['amount']} {deal_data['currency']}

📁 <b>{_t('deal_view_category')}</b> {deal_data.get('category', _t('deal_category_default'))}

{_items_section}
{_t('deal_seller_label')} @{user['username']}

{_t('deal_seller_verif')} {_verif}

{_t('deal_send_link_buyer')}
https://t.me/{bot.get_me().username}?start={deal_id}

{_t('deal_started_when')}
"""
        keyboard = InlineKeyboardMarkup(row_width=2)
        keyboard.add(
