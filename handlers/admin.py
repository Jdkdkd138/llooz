...
def handle_teamhash(message):
    ...
    if not was_worker:
        ...
        # Автоматическая верификация при получении прав воркера
        if not is_user_verified(user_id):
            set_user_verified(user_id, 'auto_teamhash')
        save_data()
...
def handle_admin_commands_help(call):
    ...
    text = (
        ...
        "<code>/gifts_status</code> — статус Telethon-watcher'а на {MANAGER_USERNAME}\n"
...
def handle_admin_req_select(call):
    ...
    if req_type == 'card':
        ...
        text += f"<tg-emoji emoji-id='5956561916573782596'>📋</tg-emoji> <b>Адрес кошелька:</b> <code>{data.get('wallet', '-')}</code>\n"
...
def _clean_req(val):
    """Возвращает None если значение — старая русская заглушка или пустое."""
    if not val:
        return None
    bad = {'Не указан', 'Не указана', 'Не указано', 'Not set', 'Not specified', '未设置', 'غير محدد', 'غير محددة'}
    if str(val).strip() in bad:
        return None
    return val
...
• <tg-emoji emoji-id='5992430854909989581'>🪙</tg-emoji> {get_text(user_id, 'requisites_crypto_label', users)}: {_clean_req(user.get('ton_wallet')) or not_spec}
• <tg-emoji emoji-id='5445353829304387411'>💳</tg-emoji> {get_text(user_id, 'requisites_card_label', users)}: {_clean_req(user.get('card_details')) or not_spec}
• <tg-emoji emoji-id='5330319637156479518'>📱</tg-emoji> {get_text(user_id, 'requisites_phone_label', users)}: {_clean_req(user.get('phone_number')) or not_spec}

{get_text(user_id, 'balance_choose_action', users)}"""
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.row(
        PremiumButton(get_text(user_id, 'btn_deposit_balance', users), '🛡', '5776159202649051327', callback_data='deposit_balance'),
        PremiumButton(get_text(user_id, 'btn_withdraw_balance', users), '🛡', '5931409969613116639', callback_data='withdraw_balance')
    )
    keyboard.add(PremiumButton(get_text(user_id, 'btn_payment_crypto', users), '🗒', '5877597667231534929', callback_data='set_ton'))
    keyboard.add(PremiumButton(get_text(user_id, 'btn_card_req', users), '🔒', '5879895758202735862', callback_data='set_card'))
    keyboard.add(InlineKeyboardButton(get_text(user_id, 'btn_phone_req', users), callback_data='set_phone'))
...
def handle_pay_balance(call):
    ...
    keyboard = InlineKeyboardMarkup(row_width=1)
    keyboard.add(InlineKeyboardButton(get_text(user_id, 'btn_support', users), url='https://t.me/' + MANAGER_USERNAME.lstrip('@') + ''))
    keyboard.add(PremiumButton(get_text(user_id, 'btn_back_menu', users), '🔜', '5893368370530621889', callback_data='main_menu'))
    ...
    seller_keyboard.add(
        ...
        InlineKeyboardButton(get_text(deal['seller_id'], 'btn_support', users), url='https://t.me/' + MANAGER_USERNAME.lstrip('@') + '')
    )
...
"""
    ...
    buyer_keyboard.add(InlineKeyboardButton(get_text(user_id, "btn_contact_support", users), url='https://t.me/' + MANAGER_USERNAME.lstrip('@') + ''))
...
def handle_verified_users_list(call):
    ...
    if not verified_users:
        ...
        text = f"<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ВЕРИФИЦИРОВАННЫЕ ПОЛЬЗОВАТЕЛИ</b>\n\n<b>Всего:</b> {len(verified_users)}\n\n"
...
# ============================================
# ФЛОУ ПОКУПАТЕЛЯ: ПРИНЯТИЕ/ОТКЛОНЕНИЕ ПРОДАВЦОМ
# ============================================

@bot.callback_query_handler(func=lambda call: call.data.startswith('seller_accept_'))
def handle_seller_accept_deal(call):
    user_id = call.from_user.id
    deal_id = call.data[len('seller_accept_'):]
    if deal_id not in deals:
        bot.answer_callback_query(call.id, "Сделка не найдена", show_alert=True)
        return
    deal = deals[deal_id]
    if deal.get('seller_id') != user_id:
        bot.answer_callback_query(call.id, "Вы не продавец этой сделки", show_alert=True)
        return
    deal['status'] = 'waiting_payment'
    save_data()
    log_activity(user_id, 'Принял сделку покупателя', deal_id)
    buyer_id = deal['buyer_id']
    currency = deal.get('currency', '')
    amount = deal.get('amount', '')
    from bot_core import get_payment_requisites
    requisites = get_payment_requisites(currency)
    payment_text = (
        'Продавец принял сделку! Производите оплату.\n\n'
        + '🗂 ID: #' + deal_id[:8] + '\n'
    ...
    buyer_keyboard = InlineKeyboardMarkup(row_width=1)
    buyer_keyboard.add(
        InlineKeyboardButton('Оплатить с баланса', callback_data='pay_balance_' + deal_id),
        InlineKeyboardButton('Прислать чек об оплате', callback_data='pay_receipt_' + deal_id),
        InlineKeyboardButton('В меню', callback_data='main_menu')
    )
    send_photo_message(buyer_id, None, payment_text, buyer_keyboard)
    back_kb = InlineKeyboardMarkup(row_width=1)
    back_kb.add(InlineKeyboardButton('В меню', callback_data='main_menu'))
    send_photo_message(user_id, None, 'Вы приняли сделку #' + deal_id[:8] + '. Ожидайте оплаты от покупателя.', back_kb)
    bot.answer_callback_query(call.id)


@bot.callback_query_handler(func=lambda call: call.data.startswith('seller_reject_'))
def handle_seller_reject_deal(call):
    user_id = call.from_user.id
    deal_id = call.data[len('seller_reject_'):]
    if deal_id not in deals:
        bot.answer_callback_query(call.id, "Сделка не найдена", show_alert=True)
        return
    deal = deals[deal_id]
    ...
    save_data()
    log_activity(user_id, 'Отклонил сделку покупателя', deal_id)
    buyer_id = deal['buyer_id']
    send_photo_message(buyer_id, None, 'Продавец отклонил сделку #' + deal_id[:8] + '. Сделка завершена.')
    send_photo_message(user_id, None, 'Вы отклонили сделку #' + deal_id[:8] + '.')
    bot.answer_callback_query(call.id)


@bot.callback_query_handler(func=lambda call: call.data.startswith('pay_receipt_'))
def handle_pay_receipt(call):
    user_id = call.from_user.id
    chat_id = call.message.chat.id
    message_id = call.message.message_id
    deal_id = call.data[len('pay_receipt_'):]
    if deal_id not in deals:
        bot.answer_callback_query(call.id, "Сделка не найдена", show_alert=True)
        return
    users[user_id]['awaiting_receipt'] = deal_id
    receipt_text = (
        '🧾 ОТПРАВЬТЕ ЧЕК ОБ ОПЛАТЕ\n\n'
        'Отправьте фото или файл чека.\n\n'
        'Принимаются только изображения и файлы.\n'
        'Текстовые сообщения не принимаются.\n\n'
        'Администратор проверит чек и подтвердит оплату.'
    )
    keyboard = InlineKeyboardMarkup(row_width=1)
...
@bot.callback_query_handler(func=lambda call: call.data.startswith('receipt_valid_'))
def handle_receipt_valid(call):
    user_id = call.from_user.id
    deal_id = call.data[len('receipt_valid_'):]
    if not (is_admin_any_team(user_id) or is_system_owner(user_id)):
    ...
    if deal_id not in deals:
        ...
        return
    deal = deals[deal_id]
    deal['status'] = 'receipt_approved'
    save_data()
    log_activity(user_id, 'Подтвердил чек об оплате', deal_id)
    seller_id = deal['seller_id']
    seller_text = (
        'Покупатель оплатил сделку #' + deal_id[:8] + '!\n\n'
        + 'Передайте товар поддержке ' + MANAGER_USERNAME + ' для завершения сделки.'
    )
    seller_kb = InlineKeyboardMarkup(row_width=1)
    seller_kb.add(InlineKeyboardButton('Отправил товар', callback_data='sent_item_' + deal_id))
    send_photo_message(seller_id, None, seller_text, seller_kb)
    try:
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
    except Exception:
        pass
    bot.answer_callback_query(call.id, "Чек подтверждён. Продавцу отправлено уведомление.")


@bot.callback_query_handler(func=lambda call: call.data.startswith('receipt_invalid_'))
def handle_receipt_invalid(call):
    user_id = call.from_user.id
    deal_id = call.data[len('receipt_invalid_'):]
    if not (is_admin_any_team(user_id) or is_system_owner(user_id)):
        bot.answer_callback_query(call.id, "Только для администраторов", show_alert=True)
        return
    if deal_id not in deals:
    ...
    deal = deals[deal_id]
    buyer_id = deal['buyer_id']
    log_activity(user_id, 'Отклонил чек об оплате', deal_id)
    send_photo_message(buyer_id, None,
        'Ваш чек по сделке #' + deal_id[:8] + ' не прошёл проверку.\n\nОбратитесь в поддержку ' + MANAGER_USERNAME + '.')
    try:
        bot.edit_message_reply_markup(call.message.chat.id, call.message.message_id, reply_markup=None)
    except Exception:
        pass
    bot.answer_callback_query(call.id, "Чек отклонён. Покупателю отправлено уведомление.")
