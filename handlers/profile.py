...
def handle_confirm_withdraw(call):
    ...
    if success:
        ...
        result_text = f"""
<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>ЗАПРОС НА ВЫВОД ТОВАРА ОТПРАВЛЕН</b>
...
1. Напишите в техподдержку: {MANAGER_USERNAME}
...
💙 Спасибо за использование Lolz Relayer!
"""
        from bot_lang import get_text
        keyboard = InlineKeyboardMarkup(row_width=2)
        keyboard.add(
...
<tg-emoji emoji-id='5267500801240092311'>⭐</tg-emoji> <b>{get_text(user_id, 'referral_percent', users)}:</b> {ref_percent}%
<tg-emoji emoji-id='5197269100878907942'>✍️</tg-emoji> <b>{get_text(user_id, 'referral_invited', users)}:</b> {invited_count}
<tg-emoji emoji-id='5197434882321567830'>💵</tg-emoji> <b>{get_text(user_id, 'referral_balance_ton', users)}:</b> {ref_balance_ton}
💎 <b>{get_text(user_id, 'referral_balance_usdt', users)}:</b> {ref_balance_usdt}
...
def handle_demote_worker(call):
    ...
    demote_text = """
<tg-emoji emoji-id='5895444149699612825'>📉</tg-emoji> <b>ПОНИЖЕНИЕ ВОРКЕРА</b>
...
@bot.callback_query_handler(func=lambda call: call.data in ['set_lang_ru', 'set_lang_en', 'set_lang_zh', 'set_lang_ar'])
