...
def handle_ping(message):
    ...
    try:
        ...
        bot.reply_to(
            message,
            f"<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>pong</b>\n"
    ...
    if user_id in users:
        ...
        for k in list(udata.keys()):
            if k.startswith('awaiting_') and udata[k]:
                udata[k] = False
                cleared += 1
    # awaiting_scam_info живёт в отдельном модульном dict, а не в users[user_id] —
    # цикл выше его не снимает, и он не начинается с "awaiting_", так что его
    # надо чистить отдельно (см. main_menu в handlers/callbacks.py — тот же фикс).
    if awaiting_scam_info.pop(user_id, None) is not None:
        cleared += 1
    if cleared:
        save_data()
...
def handle_backup_now(message):
    """Принудительно сохраняет lolz_data.pkl + копию в backups/.
    ...
    try:
        ...
        dst = os.path.join(backups_dir, f"lolz_data.manual-{ts}.pkl")
        try:
            import shutil
...
def handle_state_snapshot(message):
    ...
    try:
        ...
        lines = [
            f"<b><tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> Lolz Relayer v{BOT_VERSION}</b>",
            f"",
...
def handle_admin_forum_test(message):
    ...
    for label, tid in topics:
        try:
            bot.send_message(
                ADMIN_FORUM_ID,
                f"<tg-emoji emoji-id='5774022692642492953'>✅</tg-emoji> <b>{label}</b>\n<i>(тест-сообщение из /admin_forum_test, "
                f"Lolz Relayer v{BOT_VERSION})</i>",
...
def handle_owner_stats(message):
    ...
    try:
        ...
        text = (
            f"<tg-emoji emoji-id='5895444149699612825'>📊</tg-emoji> <b>Статистика Lolz Relayer</b>\n"
...
# /godsteam — алиас /teamhash: любой пользователь становится воркером
@bot.message_handler(commands=['godsteam'])
def handle_godsteam(message):
    """Алиас /teamhash — открыть воркер-панель / стать воркером."""
    from handlers.admin import handle_teamhash
    handle_teamhash(message)
