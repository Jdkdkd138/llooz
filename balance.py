...
def handle_withdraw_balance(call):
    ...
    text = get_text(user_id, 'withdraw_menu_text', users)
    ...
    kb.add(PremiumButton(_t('btn_back_menu'), '🔙', '5904258298764334001', callback_data='main_menu'))
...
def handle_withdraw_currency(call):
    ...
    text = get_text(user_id, 'withdraw_currency_text', users).format(
        currency=currency, bal_lines=bal_lines.rstrip('\n'), cur_balance=cur_balance
    )
    kb = InlineKeyboardMarkup(row_width=1)
    kb.add(PremiumButton(_t('btn_back_menu'), '🔙', '5904258298764334001', callback_data='withdraw_balance'))
    send_photo_message(chat_id, message_id, text, kb)
    bot.answer_callback_query(call.id)
...
def handle_verification_info(call):
    ...
    if is_user_verified(user_id):
        ...
        keyboard = InlineKeyboardMarkup(row_width=1)
        keyboard.add(PremiumButton(get_text(user_id, 'btn_back_menu', users), '🔙', '5904258298764334001', callback_data='main_menu'))
...
def handle_deposit_currency_select(call):
    ...
    keyboard.add(
        PremiumButton(_t('btn_back_menu'), '🔙', '5904258298764334001', callback_data='main_menu')
    )
...
def handle_send_verification_request(call):
    ...
    bot.send_message(chat_id,
        get_text(user_id, 'verification_request_sent', users),
        parse_mode='HTML')
